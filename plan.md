...
> **진행 상황 (2026-08-05)**: Step 1~3 완료 / Step 4·5·6·7 대기
> (Step 5는 한 번 완료했으나 담당 경계를 넘은 것이어서 2-2로 반환 — [WORKLOG.md](WORKLOG.md) 참조)

### 데이터 준비 + 시각화 (35점)
- [x] 데이터셋을 **Pandas와 Polars 양쪽으로 로딩**하여 결과 비교 — Step 1
- [x] **결측치·중복 처리** 및 기본 EDA 수행, 결과 출력 — Step 2·3
- [ ] **Seaborn 정적 차트 1개 이상** + **Plotly 인터랙티브 차트 1개 이상** — Step 5 (2-2 이은혜)
  (분포·상관관계·그룹 비교 중 택일, **제목·축 레이블 필수**)

### 통계 분석 + ML Pipeline (45점)
- [ ] 기술통계(평균·표준편차·분위수) 산출 — Step 3 (2-3 안중범)
- [ ] 변수 간 **상관계수** 계산·출력 — Step 3 (2-2 이은혜)
- [ ] **`scipy.stats.ttest_ind`로 t-test** 수행 및 **p-value 해석** 포함 — Step 4
- [ ] **`sklearn.pipeline.Pipeline`** 객체로 전처리 + 모델 학습 구성 — Step 6
- [ ] **평가지표(정확도·F1 등) 출력** — Step 6
- [ ] **joblib으로 모델 파일 저장** — Step 6

### 자동화 + 발표 (20점)
- [ ] 분석 결과를 **report.md로 자동 생성** (스크립트로 생성해야 함) — Step 7
  (각 단계가 `outputs/results/*.json`을 남기고 있어 조립만 하면 된다)
- [ ] 팀별 발표 5분

### 완성도 (10점)
- [x] **주석 처리 누락 시 감점** → 모든 코드에 주석 충실히 작성
  (작성된 스크립트 4개는 반영 완료, 이후 스크립트도 동일 기준 유지)
...
1. **타깃 누수(target leakage)**: 현금 팁은 기록되지 않아 `tip_amount`가 결제수단을 그대로 노출
   (팁 > 0 비율: 카드 90.18% / Flex Fare 9.42% / 현금 0.01% — **현금만 구조적으로 미기록**).
   → 누수 컬럼 8개를 **피처에서 제외**하고 근거를 보고서에 명시
   (`tip_amount`, `total_amount`, `congestion_surcharge`, `Airport_fee`, `RatecodeID`,
   `passenger_count`, `store_and_fwd_flag`, `VendorID`)
...
5. **Flex Fare 요금의 끝자리 패턴**: 요금이 $0.50 배수인 비율이 Flex Fare 2.70% vs 카드 27.19% / 현금 23.66%.
...
Step 1~7로 재편하면서 담당자 이름이 빠져 "누가 무엇을 받는지"를 알 수 없게 됐다.
`pj.txt`의 분업과 아래처럼 대응한다.

| Step | 파일 | 담당 | `pj.txt` 대응 | 상태 |
|---|---|---|---|---|
| 1 | `01_load_compare.py` | 정민교 | 2-1 (Pandas·Polars 로딩 비교) | 완료 |
| 2 | `02_preprocess.py` | 정민교 | 2-1 (결측·중복, subset, 파생변수) | 완료 |
| 3 | `03_eda.py` | 정민교 | 2-1 (피처 확정까지) | 완료 |
| 3 | 기술통계 | **안중범** | 2-3 (기술통계 산출) | 대기 |
| 3 | 상관계수 | **이은혜** | 2-2 (상관관계로 subset·파생변수 추출) | 대기 |
| 4 | `04_stats.py` | **안중범** | 2-3 (t-검정) | 대기 |
| 5 | `05_visualize.py` | **이은혜** | 2-2 (시각화) | 대기 |
| 6 | `06_ml_pipeline.py` | 장소영 · 정민교 (**확인 필요**) | — | 대기 |
| 7 | `07_report.py` | 이재호 (**확인 필요**) | — | 대기 |

Step 6·7의 배정은 `pj.txt`에 명시가 없어 남은 인원으로 추정한 것이다. **팀에 확인 후 확정할 것.**

> **2-1이 남긴 주의**: Step 5와 Step 3의 기술통계·상관계수를 2-1이 한 번 만들었다가
> 담당 경계를 넘은 것이어서 되돌렸다(PR #1 리뷰). `pj.txt`의 "Merge 후 다음 사람에게 토스" 규칙대로
> 받는 시점에 빈 상태여야 토스가 성립하기 때문이다.
>
> `clean()`의 **이상값 제거만은 예외로 2-1이 유지**한다. `cleaned.parquet`이 Step 3~6 전체의 입력이라
> 이것만 빼면 뒤 단계가 이상치 낀 데이터를 받게 된다. 2-2와 협의 후 조정할 것.
...
- 이상값 제거: fare_amount > 0, 0 < trip_distance < 200, 0 < trip_duration_min < 180
  ...
  → 세는 조건과 지우는 조건은 서로의 정확한 부정이어야 한다 (경계값 누락 방지)
  → **요금에 상한이 없는 것은 의도한 비대칭** — 고액 요금은 장거리·정액요금으로 실제 발생 가능
- 파생변수 4개: `trip_duration_min`, `pickup_hour`, `day_of_week`, `is_weekend`
  (이 중 `is_weekend`는 사람이 읽는 요약용. `day_of_week`에서 완전히 유도되므로 **모델 피처에서는 제외**)
...
2. **결제수단별 기술통계** — 그룹 간 비교 (EDA의 핵심)
3. **시간 패턴** — 시간대별·요일별 결제수단 구성비 (표로)
4. **지역 패턴** — 승·하차 상위 10개 구역의 결제수단 구성비.
   `PULocationID`·`DOLocationID`는 지역 코드이므로 **상관계수가 아니라 교차표로** 본다.
   132번(JFK 공항)은 평균 요금 4배·거리 7배·현금 27%로 뚜렷이 다르다 —
   Step 6에서 `is_airport`(132 JFK, 138 LaGuardia) 파생변수를 검토할 것
5. **누수 진단** — (a) 팁 > 0 비율(카드 90.18% / Flex Fare 9.42% / 현금 0.01%),
   (b) 요금 끝자리 $0.50 배수 비율(Flex Fare 2.70% / 현금 23.66% / 카드 27.19%),
   (c) `VendorID`별 구성비 — **VendorID=6은 100% Flex Fare이므로 누수, 피처에서 제외**
6. **피처 화이트리스트 확정** — 위 결과를 근거로 쓸 컬럼 7개 / 뺄 컬럼 8개를 파일로 확정.
   **범주형 컬럼은 타깃과 교차표로 검증한 뒤에만 화이트리스트에 넣는다**

> **이 단계가 다루지 않는 것**: 전체 기술통계(2-3 안중범)와 상관계수(2-2 이은혜).
> 한때 `03_eda.py`에 함께 있었으나 분업 경계를 넘은 것이어서 반환했다(PR #1 리뷰).
> 둘 다 `features.json`에 쓰이지 않으므로 Step 6의 입력에는 영향이 없다.

**산출물**
- `outputs/results/eda.json` / `eda.md`
- **`outputs/results/features.json`** — Step 6이 읽어 쓸 피처 화이트리스트

**검증**: `features.json`에 누수 컬럼 8개가 하나도 포함되지 않았는지 (`assert`)

---

### Step 4 — 통계 검정 (`04_stats.py`)
**Task**
- **t-test 2건을 나란히 수행한다.** `scipy.stats.ttest_ind(..., equal_var=False)` (Welch's t-test)
  1. 신용카드 vs 현금의 `fare_amount` — **p=0.015로 유의하지만 Cohen's d=−0.005로 사실상 차이 없음**
  2. Flex Fare vs 신용카드의 `fare_amount` — **p≈0, d=0.315로 실질적 차이 있음**

  1번만 제시하면 "p<0.05이므로 카드와 현금의 요금은 다르다"로 읽혀 정반대 인상을 준다.
  둘을 붙여 놓아야 아래 "유의성과 효과 크기의 구분"이 한 장에서 드러난다.

  > 팀 리포트 PDF 4.3절은 "Flex Fare vs 카드의 `trip_distance`"(t=85.13, d=0.091)를 했다.
  > 이 수치는 재현 검증을 통과했으나 효과크기가 0.2에도 못 미치므로,
  > 발표에서는 위 2번(요금, d=0.315)을 주 근거로 쓰는 편이 낫다.
...
4. 통계: 두 조합을 나란히 — 카드 vs 현금 요금(p=0.015로 유의하나 Cohen's d=-0.005로 무의미) /
   Flex Fare vs 카드 요금(p≈0, d=0.315로 실질적). 유의성과 효과크기의 차이가 한 장에 드러난다

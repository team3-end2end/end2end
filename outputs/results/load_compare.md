...
| 항목 | Pandas | Polars | 차이 |
|---|---|---|---|
| 로딩 시간 (중앙값) | 0.232초 | 0.157초 | Polars가 1.48배 빠름 |
| 측정값 전체 | 0.232, 0.185, 0.238 | 0.259, 0.157, 0.135 | |
...
| 항목 | Pandas | Polars | 차이 |
|---|---|---|---|
| 전처리 시간 (중앙값) | 0.172초 | 0.039초 | **Polars가 4.46배 빠름** |
| 측정값 전체 | 0.191, 0.172, 0.172 | 0.040, 0.039, 0.036 | |
| 집계 결과 일치 | ✅ 동일 | | |
...
근거:
- 로딩 단계의 차이(1.48배)는 선택 근거로 삼기에 작다.
  반면 실제 전처리 작업에서는 **4.46배** 차이가 났고, 같은 결과를 낸다는 것도 확인했다.

...
#
# 3회에서 15회로 올린 이유: 3회로는 실행할 때마다 로딩 배수가 1.00x~2.56x로 흔들려
# 어떤 값을 문서에 적어도 다음 사람이 재현하지 못했다(PR #1 리뷰). 표본을 늘려 편차를 줄인다.
# 그래도 로딩 배수는 환경(코어 수·디스크)에 따라 달라지므로 단독 판단 근거로 쓰지 않는다.
N_RUNS = 15
...
def measure(load_fn, n_runs: int = N_RUNS) -> tuple[object, list[float]]:
    ...
    for _ in range(n_runs):
        # 이전 회차의 프레임(581MB)을 먼저 버린다.
        # `df = load_fn()`은 대입이 호출 완료 후에 일어나므로, 이 줄이 없으면
        # 2회차부터는 프레임 2개가 동시에 존재하는 상태에서 측정하게 된다.
        # 그러면 1회차만 조건이 달라져 배수가 실행마다 크게 흔들린다.
        df = None
        gc.collect()
...
- 반면 실제 전처리 작업에서는 **약 3~4배**(코어 수에 따라 다름 — 8코어에서 2.7~3.7배,
  12코어에서 4.2배, 이번 실행 {tf['speedup']:.2f}배) 차이가 났고, 같은 결과를 낸다는 것도 확인했다.
